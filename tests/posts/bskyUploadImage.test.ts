...
describe('bskyUploadImage', () => {
	...
	it('should upload image successfully', async () => {
		...
		const result = await bskyUploadImage(
			'https://bsky.social',
			imagePath,
			sessionData,
			imageMimeType
	...
	it('should throw an error if image size is too large', async () => {
		...
		await expect(
			bskyUploadImage('https://bsky.social', imagePath, sessionData, imageMimeType)
		).rejects.toThrow('Image size (2000000) is too large. Max size is 1000000 bytes.');
	});
});
